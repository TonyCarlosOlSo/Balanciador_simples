...
int main() {
    ...
    // Abre o arquivo para salvar os resultados (ou cria se não existir)
    file = fopen("resultados.txt", "a"); // Modo "append" para adicionar resultados
    if (file == NULL) {
    ...
    if (checkBalance(expression)) {
        ...
        fprintf(file, "Expressão: \"%s\" -> Balanceada\n", expression);
    } else {
        printf("A expressão NÃO está balanceada.\n");
        fprintf(file, "Expressão: \"%s\" -> Desbalanceada\n", expression);
    }
